...
export function handleTransfer(event: Transfer): void {
  ...
  // case where direct send first on NBU withdrawls
...
export function handleSync(event: Sync): void {
  ...
  let trackedLiquidityNBU: BigDecimal
  if (bundle.nbuPrice.notEqual(ZERO_BD)) {
    trackedLiquidityNBU = getTrackedLiquidityUSD(pair.reserve0, token0 as Token, pair.reserve1, token1 as Token).div(
      bundle.nbuPrice
    )
  } else {
    trackedLiquidityNBU = ZERO_BD
  }
  ...
  nimbus.totalLiquidityNBU = nimbus.totalLiquidityNBU.plus(trackedLiquidityNBU)
  nimbus.totalLiquidityUSD = nimbus.totalLiquidityNBU.times(bundle.nbuPrice)
